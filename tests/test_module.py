...
def test_red_herring_files(data_paths, config) -> None:
    ...
    # Generate puzzles and get the red herring path
...
def test_scores(data_paths, config) -> None:
    """Test the evaluation scores of a dataset of zebra puzzles.

    Puzzles are generated by the data_paths fixture.
    """
...
def test_responses(data_paths, config) -> None:
    ...
    responses_path_str = f"{data_folder}/{theme}/{n_objects}x{n_attributes}/{n_red_herring_clues_evaluated}rh/responses/{model}/"
    responses_path_file_str = responses_path_str + "zebra_puzzle_0_response.json"

    with open(responses_path_file_str, "r") as f:
        response = f.read()
